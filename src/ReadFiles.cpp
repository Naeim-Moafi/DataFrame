...
void DF::DataFrame::fill_data(std::shorts::V_string const& lines, char delim, bool is_first_col_header, std::shorts::V_string v_hdrs)
{
    ...
    for(unsigned long long i_col{0}; i_col < n_cols; ++i_col)
    {
        ...
        else if(v_hdrs.size() > 0)
        {
            ...
            {
                fmt::println(stderr, "Error: number of provided headers does not match with the number of columns in the data");
                exit(EXIT_FAILURE);
            }
        }
        else
    ...
    for(unsigned long long i_col{0}; i_col < n_cols; ++i_col)
    {
        ...
        for(unsigned long long i_row{is_first_col_header}; i_row < n_rows; ++i_row)
        {
            ...
            {
                fmt::println(stderr, "Error: inconsistent number of columns, check row {}", i_row + 1);
                exit(EXIT_FAILURE);
            }
...
void DF::DataFrame::head(unsigned long long n)
{
    if(n > n_rows)
    {
        fmt::println("Warning: number of columns requested to be printed is bigger than number of available rows in data");
        n = n_rows;
    }

    for(auto const& curr_hdr : headers)
    {
        fmt::print("{:10} ", curr_hdr);
    }
